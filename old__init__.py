...
apiList=['/api/OTPLogin', '/api/UserLogin', '/api/StatusTrack','/api/ActiveCases',
    ...
    '/api/tSignatureUse','/api/tQueryReply','/api/EnhanceFinal','/api/ClarificationReply','/api/pQueryReply',
    '/api/pSignature','/api/IncidentCreate','/api/UserProfile','/api/tSignatureCreate','/api/get_from_query','/api/get_from_query1','/api/get_from_name','/api/get_portal_submitted','/api/update_ssdoc']
    ...
    try :
        ...
        if data:
            ...
            query1="""select pa.PatientID_TreatmentID,pa.insname,pa.admission_date,pa.p_sname,st.status FROM preauth pa \
                    ...
                    STR_TO_DATE(pa.dischargedate, '%s')>= STR_TO_DATE('%s', '%s') AND """%(dateformat,today,dateformat)
            ...
            finalResponse['status']="failed"
            finalResponse['message']='Hospital ID is not present in hospital_employee table with respect to Mobile Number'
            # finalResponse['count']=len(data)
    except Exception as e:
        print (e)
        finalResponse['status']="failed"
    ...
    try :
        ...
        if data:
            ...
            if data:
                ...
                finalResponse['status']="failed"
                finalResponse['message']='UTR_No and status Are Not Present from settlementutrupdate table because status must in C and CR with respect to HospitalID'
...
def statusTrack () :
    ...
    try :
        ...
        if data:
            ...
            finalResponse['status']="failed"
            finalResponse['message']='PatientID_TreatmentID not present in status_track table with respect to patientId OR Record does not found with respect to PatientID_TreatmentID'
...
def tsignatureusecancel():
    ...
    try :  
        ...
        if data:
            ...
            finalResponse['status']="failed"
            finalResponse['message'] ='status is not present Signature request table with respect to Mobile number,PatientID_TreatmentID And HospitalID '
...
def tsignatureuse():
    ...
    try :
        ...
        if data:
            ...
            finalResponse['status']="failed"
            finalResponse['message']='Record does not found in signature_request table with respect to Mobile And PatientID_TreatmentID'
    ...
    try :  
        ...
        if data :
            ...
            if type1 =='Claim' :
                ...
                VALUES ( '%s' , '%s' , '%s' , '%s' , '%s' , '%s','%s' ,'0','0','%s')"""%(hId,pId,qFlag,type1,cstatus,desc,amount,cdate)
                print(query)
                cur.execute(query)
                ...
                if cstatus=='Enhancement Information Awaiting':
                    ...
                    VALUES ( '%s','%s','%s','%s','%s','%s','%s','0','0','%s')"""%(hId,pId,qFlag,type1,cstatus,amount,desc,cdate)
                    print(query)
                    cur.execute(query)
    ...
    if flag =='e' or flag == 'E':
        try :
            ...
            if data :
                ...
                VALUES ( '%s' , '%s' , '%s' , '%s' ,'%s','%s' ,'0','0','%s') """%(hId,pId,flag,type1,desc,amount,cdate)
                print(query)
                cur.execute(query)
            ...
            if data:
                ...
                if data:
                    ...
                    if(len(list1) > 1):
                        ...
                        "%(srno+1,pId,newtrackingno,desc,urg,'open',reportby,reportedto,emailId,phoneNo,issueDesc)
                        print(query)
                        cur.execute(query)
                        finalResponse['status']="success"
                else:
        ...
        if data:
            ...
            if data:
                ...
                if data:
                    ...
                    validity,path,remarks) values('%s',%s,'%s','%s','Requested','%s','30','%s','%s') """%(ptreatmentid,int(mNo),pName,today1,hId,path,remark)
                    print(query)
                    cur.execute(query)
                    finalResponse['status']="Success"
                    finalResponse['message']='Request generated successfully for Patient. Validity is 30 min only'
                ...
                finalResponse['status'] ='Failed'
                finalResponse['message'] ='PatientID_TreatmentID Not Found from preauth table with respect to Mobile Number'  
        ...
        if len(query) > len("update patient_master set "):
            ...
            if relativeList is not None:
                for relative in relativeList:
                    ...
                    values('%s','%s','%s','%s')"""%(mNo,relative_dic['r_name'],relative_dic['r_mobile'],relative_dic['r_relation'])
                    print(query)
                    cur.execute(query)   
                    conn.commit() 
        else:
...
def userDoc():
    ...
    conn=None
    if r_type== 'Insurance':
        if ((len(files) != len(Insurer_Name)) or (len(files) !=len(InsurerID)) or (len(files) !=len(Expiry_date)) or (len(files) !=len(Sum_insurance))):
            finalResponse['status']='failed'
            finalResponse['message']='length of files, Insurer_Name,InsurerID,Expiry_date,Sum_insurance should be same'
            return jsonify(finalResponse)
        ...
        if r_type== 'Insurance':
            ...
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    query = """insert into patient_ndoc(mobile,`type`,path,Insurer_Name,InsurerID,PolicyID,Expiry_date,Sum_insurance) values ('%s','%s','%s','%s','%s','%s','%s','%s')""" %(mobile_no,r_type,filename,Insurer_Name[index],InsurerID[index],PolicyID[index],Expiry_date[index],Sum_insurance[index])
                    print(query)    
                    cur.execute (query )
                    index=index+1
            ...
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    mList.append(localRespons)
                    # f_path=app.config['UPLOAD_FOLDER'])+"/"+filename
                    query = """insert into patient_mdocs(mobile,`type`,path,cdate,`desc`) values (%s,'%s','%s','%s','%s')""" %(int(mobile_no),r_type,filename,cdate[index],desc[index])
                    print(query)    
                    cur.execute (query )
                    index=index+1
...
def hospitalList():
    ...
    try :
        ...
        if data:
            ...
            finalResponse['status']='failed'
            finalResponse["message"]='Hospital_Name,HospitalID Are not present in hospital Table when status equal to 1'
...
def claimdeficiency():
    ...
    try :
        ...
        if fromdate != '' and todate != '':
            ...
            and STR_TO_DATE('%s', '%s') AND p.HospitalID='%s'"""%(dateformat,fromdate,dateformat,todate,dateformat,hospitalId)
        else:
            query="""SELECT p.HospitalID,p.PatientID_TreatmentID AS PatientID,p.preauthNo AS PreAuthNo, p.MemberId,p.p_sname AS PatientName,p.admission_date AS AdmissionDate, \
            p.dischargedate AS DischargeDate,p.status AS `Status`,c.PatientID_TreatmentID,p.Total_expected_cost AS ExpectedAmount \
            FROM  preauth p left JOIN claim c ON c.PatientID_TreatmentID = p.PatientID_TreatmentID WHERE \
...
def EmailSend():
    ...
    try :
        ...
        if data:
            ...
            finalResponse['status']="failed"
            finalResponse['message']='Record does not found in mail_configration table with respect to InsurerID And HospitalID'   
    except Exception as e:
        print (e)
        finalResponse['status']='failed',
        finalResponse["message"]='Something went wrong.Pls try after Some time or contact Support'
...
#                 }
#             )    
#     if request.form.get('patientid_treatmentid') !=None:
#         patientid_treatmentid = request.form['patientid_treatmentid']
#     if request.form.get('hospitalId') !=None:
#         hospital_id = request.form['hospitalId']    
#     if request.form.get('status') !=None:
#         status = request.form['status']    
    
#     if patientid_treatmentid == '' and hospital_id =='' and status =='':
#         return jsonify(
#                 {
#                 'status' : 'failed',
#                 'message':'mobile number field is empty'
...
#         # conn=mysql.connect()
#         # cur = conn.cursor()
#         tAlert = triggerAlert(patientid_treatmentid, hospital_id, status)
#         fetchsms = fetch_sms_and_push_notification_records(patientid_treatmentid, hospital_id, status)
#         fetchcontact = fetch_p_contact_no(patientid_treatmentid)
#         fetchhospcontact = fetch_hosp_contacts(hospital_id)
#         fetchtoken = fetch_token_list(mobile_no_list, type)
#         bsendsms = send_sms_and_notifications(mob_token_list, result)
#         writealert = write_to_alert_log(data_dict)
#         print("success")
